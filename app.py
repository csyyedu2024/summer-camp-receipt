...
# ==========================
# ⚠️ 【重要】請將下方替換成您總表「真實的網址」(帶有 /edit 的那串)，不要用 CSV 網址！
SHEET_URL = "https://docs.google.com/spreadsheets/d/1d1H1ofnf2NS0EJf5eaDH6XrmPT0k9tvk_7jwKjlPUtE/edit?resourcekey=&gid=185848851#gid=185848851"
...
if not df.empty:
    ...
    # 只要有輸入電話，就自動開始查詢 (拿掉原本的查詢按鈕，避免阻擋後續的表單)
    if search_phone:
                    ...
                    if len(five_digits.strip()) < 4:
                        ...
                        with st.spinner("系統正在將資料寫入總表，請稍候..."):
                            try:
                                ...
                                if "匯款後五碼" not in sheet_headers:
                                    st.error("⚠️ 寫入失敗！請確認您的 Google 總表第一列有「匯款後五碼」這個標題。")
                                else:
                                    col_idx = sheet_headers.index("匯款後五碼") + 1
                                    rows_to_update = user_data['真實列數'].tolist()
                                    
                                    # 依序更新該家長所有孩子的「匯款後五碼」欄位
                                    for r in rows_to_update:
                                        # 加上單引號，避免 Google 把 00123 開頭的 0 吃掉
